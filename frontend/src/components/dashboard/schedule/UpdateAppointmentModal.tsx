...
export function UpdateAppointmentModal({
    ...
    const {
        ...
        selectedClinic,
        selectedRoom
    } = useUpdateAppointmentModal(appointment, onClose)

    if (!open) return null


    return (
        <div className="fixed inset-0 z-[60] flex items-start justify-center bg-black/40 px-4 py-8 sm:py-12 overflow-y-auto">
            <div className="w-full max-w-xl rounded-lg bg-white shadow-xl my-8">
                <div className="flex items-center justify-between bg-[#0000AC] px-6 py-4 text-white">
                    <span className="text-base font-semibold">Update Appointment</span>
                    <button
                        type="button"
                        onClick={onClose}
